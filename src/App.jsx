...
// ... existing code ...
    ...
    useEffect(() => {
        ...
        const pollingId = setInterval(async () => {
            ...
            // --- MODIFICATION: We pass `callApi` as a prop, so we don't need to define it here ---
            const result = await callApi('checkPaymentStatus', { paymentReference: paymentReference, voterId: voterId }, true); // Pass true to suppress global loading spinner
            ...
            // If still PENDING, the interval will just run again
    ...
    // --- MODIFICATION: Added `callApi` to dependency array ---
...
// --- REMOVED: This component is no longer needed as it's replaced by the PaymentStatusPage -> ThankYouPage flow ---
...
export default function App() {
  ...
  // --- REMOVED: This state is no longer needed ---
  // const [isConfirmationModalOpen, setIsConfirmationModalOpen] = useState(false);
  
  // --- ADDED: New state to store the payment reference for polling ---
  ...
  // --- MODIFICATION: Added `suppressLoading` flag ---
  // This stops the global loading spinner from showing during polling
  const callApi = async (action, data = {}, suppressLoading = false) => {
    if (!suppressLoading) {
        setIsLoading(true);
    }
    // setError(''); // Don't clear error on polling, as it might wipe a "Failed" message
    try {
      ...
      if (result.status === 'error') {
        if (!suppressLoading) setError(result.message);
        return null;
      }
      return result;
    } catch (err) {
      console.error("API Call Failed:", err);
      if (!suppressLoading) setError('Could not connect to the voting server. Please check your network and try again.');
      return null;
    } finally {
      if (!suppressLoading) {
        setIsLoading(false);
...
// ... existing code ...
  ...
  // --- MODIFICATION: This function now triggers the polling page ---
  const handleInitiatePayment = async () => {
    ...
    if (result && result.paymentReference) {
      // 1. Set the reference ID we get back from the backend
      setPaymentReference(result.paymentReference); 
      // 2. Close the review modal
      setIsReviewModalOpen(false);
      // 3. Change the page to show the new polling component
      setPage('paymentStatus');
    } else if (result) {
        // Fallback in case the backend returns success but no reference (old API version?)
        setError("Payment initiated, but no reference was returned. Please contact support.");
    }
    // If `result` is null, the `callApi` function already set the error.
  };
...
// ... existing code ...
  ...
  // --- MODIFICATION: Added resets for new state ---
  const resetToHome = () => {
    setVoterId('');
    setVoterName('');
    setVoteAmounts({});
    ...
    setAuthMode('login');
    setPage('auth');
    // --- REMOVED: No longer need to close this modal ---
    // setIsConfirmationModalOpen(false); 
    // --- ADDED: Clear the payment reference ---
    setPaymentReference('');
  };
  
  // --- MODIFICATION: Added new cases for 'paymentStatus' and 'thankYou' ---
  const renderPage = () => {
    switch(page) {
      ...
      // --- ADDED: New page for polling payment status ---
      case 'paymentStatus': return <PaymentStatusPage {...{ paymentReference, handleGoToAuth: resetToHome, voterName, setPage, voterId, callApi }} />;
      
      // --- ADDED: New page for final success message ---
      case 'thankYou': return <ThankYouPage {...{ resetToHome, voterName }} />;
  ...
  return (
    <div className="bg-gray-50 min-h-screen flex flex-col items-center justify-center p-4 sm:p-6 lg:p-8">
      ...
      {/* This modal is still used for reviewing before payment */}
      <ReviewModal 
      ...
      {/* --- REMOVED: The old ConfirmationModal is no longer rendered here --- */}
